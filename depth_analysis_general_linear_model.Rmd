...
As per the OLS analysis, I'll only be including Mc2. Additionally, I will be using the average of fcs and fcp: fc.
```{r}
...
Next, we cross-validate to find the best value for lambda. This allow us to determine which features are most important in predicting depth, since lambda controls how aggressive the lasso model's feature selection is. 
...
Using our model and the value of lambda that minimizes our error, we can make some predictions and evaluate the model.
...
```{r}
write.csv(test$baseline_resids, file = "C:/Users/riley/ds_portfolio/baseline_resids.csv")
write.csv(test$glm.resids, file = "C:/Users/riley/ds_portfolio/fourfeatureOLS_resids.csv")
```
